...
appAxiosClient.interceptors.request.use(config => config, error => Promise.reject(error));
...
async function PUT<T>(url: string, config?: Omit<AxiosRequestConfig, "url">): ReturnType<typeof request> {
  return request<T>({...config, method: "PUT", url})
}

async function POST<T>(url: string, config?: Omit<AxiosRequestConfig, "url">): ReturnType<typeof request> {
  return request<T>({...config, method: "POST", url})
}

async function DELETE<T>(url: string, config?: Omit<AxiosRequestConfig, "url">): ReturnType<typeof request> {
